                               ...
                               struct ompi_communicator_t *comm, mca_coll_base_module_t *module)
{
    ...
    if (root == w_rank) {
        ...
        up_displs = calloc(up_size, sizeof(int));
        ...
        for (w_peer = 0; w_peer < w_size; ++w_peer) {
            ...
            up_peer_ub[up_peer] = displs[w_peer] + rcounts[w_peer] > up_peer_ub[up_peer]
            ...
            up_displs[up_peer] = 0 == up_displs[up_peer] || displs[w_peer] < up_displs[up_peer]
                                     ? displs[w_peer]
                                     : up_displs[up_peer];
        }
        ...
        for (up_peer = 0; up_peer < up_size; ++up_peer) {
            ...
            if (!need_bounce_buf
                && up_rcounts[up_peer] < up_peer_ub[up_peer] - up_peer_lb[up_peer]) {
                need_bounce_buf = 1;
                break;
            }
        }
        ...
        if (need_bounce_buf) {
             ...
             * - If reordering is not needed, i.e. mapped by core, receive data directly at the
             * correct displacements on the output buffer.
             * - Otherwise receive the data on the output buffer immediately after the data from
             * local local peers. Later reorder the data with the help of the temp buffer.
             */
            for (up_peer = 0; up_peer < up_size; ++up_peer) {
        ...
        if (need_bounce_buf) {
            ...
            for (int i = 0; i < w_size; ++i) {
                ...
                w_peer = topo[2 * i + 1];
                rcount = (size_t) rcounts[w_peer];

                ompi_datatype_copy_content_same_ddt(rdtype, rcount,
                                                    (char *) rbuf
                                                        + (size_t) displs[w_peer] * rdsize,
                                                    bounce_buf + offset);
                offset += rdsize * rcount;
            }
        }
    ...
    size_t *low_data_size = NULL, sdsize = 0, send_size = 0;
    ...
    for (int i = 0; i < low_size; ++i) {
        low_rcounts[i] = (int) (low_data_size[i] / datatype_size);
        low_displs[i] = i > 0 ? low_displs[i - 1] + low_rcounts[i] : 0;
