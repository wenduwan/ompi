 ...
 * 1. Root:
 *      a. Send data to local peers (Low Scatterv)
 *      b. If necessary, reorder and sort data (See discussion below)
 *      c. Send data to other node leaders (Up Scatterv)
 ...
 *      b. Receive data from the root (Up Scatterv)
 ...
 *
 * Note on reordering:
 * In Up Scatterv, reordering the send buffer can be avoided if and only if both of following
 * conditions are met:
                            ...
                            mca_coll_base_module_t *module)
{
    ...
    if (root == w_rank) {
        ...
        up_peer_lb = malloc(up_size * sizeof(int));
        ...
        for (w_peer = 0; w_peer < w_size; ++w_peer) {
            ...
            if (!need_bounce_buf && root_up_rank != up_peer && 0 < w_peer
            ...
            up_peer_lb[up_peer] = displs[w_peer] < up_peer_lb[up_peer] ? displs[w_peer]
                                                                       : up_peer_lb[up_peer];

            up_peer_ub[up_peer] = displs[w_peer] + scounts[w_peer] > up_peer_ub[up_peer]
            ...
            up_displs[up_peer] = displs[w_peer] < up_displs[up_peer] ? displs[w_peer]
                                                                     : up_displs[up_peer];
        }

        /* If the data is not placed contiguously on send buf, then we will need temp buf to store
        ...
        for (up_peer = 0; up_peer < up_size; ++up_peer) {
            ...
            if (!need_bounce_buf
                && up_scounts[up_peer] < up_peer_ub[up_peer] - up_peer_lb[up_peer]) {
                need_bounce_buf = 1;
                break;
            }
        }
        ...
        if (need_bounce_buf) {
            ...
            for (int i = 0; i < w_size; ++i) {
                ...
                w_peer = topo[2 * i + 1];
                scount = (size_t) scounts[w_peer];

                ompi_datatype_copy_content_same_ddt(sdtype, scount,
                                                    bounce_buf + offset,
                                                    (char *) sbuf + (size_t) displs[w_peer] * sdsize);
                offset += sdsize * scount;
            }
        ...
        /* Up Scatterv */
        /* rbuf needs to be the same one used in node leaders? */
        up_comm->c_coll->coll_scatterv(reorder_sbuf, up_scounts, up_displs, sdtype,
                                       rbuf, rcount, rdtype, 
                                       root_up_rank, up_comm, up_comm->c_coll->coll_scatterv_module);
    ...
    if (root_up_rank == up_rank) {
        /* Low Scatterv */
        low_comm->c_coll->coll_scatterv(NULL, NULL, NULL, NULL, rbuf, rcount, rdtype,
                                        root_low_rank, low_comm, low_comm->c_coll->coll_scatterv_module);
        return OMPI_SUCCESS;
    }

    /* #################### Other node followers ########################### */
    size_t *low_data_size = NULL, rdsize = 0, receive_size = 0;
    ...
    low_data_size = malloc(low_size * sizeof(size_t));
    if (!low_data_size) {
        err = OMPI_ERR_OUT_OF_RESOURCE;
    ...
    for (int i = 0; i < low_size; ++i) {
        rsize += low_data_size[i];
        max_data_size = low_data_size[i] > max_data_size ? low_data_size[i] : max_data_size;
    }
    ...
    for (int i = 0; i < low_size; ++i) {
        low_scounts[i] = (int) (low_data_size[i] / datatype_size);
    ...
    /* Up Scatterv */
    up_comm->c_coll->coll_scatterv(NULL, NULL, NULL, NULL,
                                   (void *) tmp_buf, total_up_scount, temptype,
                                   root_up_rank, up_comm, up_comm->c_coll->coll_scatterv_module);

    /* Low Scatterv */
    low_comm->c_coll->coll_scatterv((void *) tmp_buf, low_scounts, low_displs, temptype,
                                    rbuf, rcount, rdtype,
                                    root_low_rank, low_comm, low_comm->c_coll->coll_scatterv_module);
